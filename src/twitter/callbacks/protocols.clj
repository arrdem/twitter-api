(ns twitter.callbacks.protocols
  ...
  - single/streaming: whether the call results in an infinite stream or a finite response body The
    function signatures for the callbacks are:
...
(defprotocol EmitCallbackList
  (emit-callback-list [this]))

(defprotocol AsyncSyncStatus
  (get-async-sync [this]))

(defprotocol SingleStreamingStatus
  (get-single-streaming [this]))
  ...
  (emit-callback-list [this]
    (merge req/*default-callbacks*
           {:part (fn [response baos]
                    ((:on-bodypart this) response baos)
                    [baos :continue])})))

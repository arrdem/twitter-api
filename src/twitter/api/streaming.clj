...
(def ^:dynamic *streaming-api*
  (ApiContext. "https" "stream.twitter.com" "1.1"))
...
(defmacro def-twitter-streaming-method
  ...
  `(def-twitter-method ~name ~verb ~resource-path
     :api ~*streaming-api*
     :callbacks (get-default-callbacks :async :streaming)
     ~@rest))
...
(def ^:dynamic *user-stream-api*
  (ApiContext. "https" "userstream.twitter.com" 2))
...
(def ^:dynamic *site-stream-api*
  (ApiContext. "https" "sitestream.twitter.com" "1.1"))
